...
public class PackageResourceLoader {
	...
	protected Set<File> retrieveMatchingFiles(File rootDir) throws IOException {
		if(!rootDir.exists()){
			...
			if(logger.isDebugEnabled()){
				logger.debug("Skipping [" + rootDir.getAbsolutePath() + "] because it does not denote a diretory");
			}
	...
	protected void doRetrieveMatchingFiles(File dir, Set<File> result) throws IOException {
		...
		if(dirContents == null){
			if(logger.isWarnEnabled()){
				logger.warn("Could not retrieve contents of diretory [" + dir.getAbsolutePath() + "]");
			}
			return;
		}
		...
		for(File content : dirContents){
			if(content.isDirectory()){
				if(!content.canRead()){
					if(logger.isDebugEnabled()){
						logger.debug("Skipping subdirectory [" + dir.getAbsolutePath() + "] because the applicaiton is not allowed to read the directory");
					}
				}else{
